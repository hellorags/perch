...
export default function Perch() {
  ...
  const citiesWithShops = useMemo(() => SHOPS.map((s) => ({ ...s, distance: haversineMiles(city, s), score: workScore(s) })).filter((s) => s.city === cityKey), [cityKey, city]);
    ...
    >
      ...
      <div className="max-w-2xl mx-auto px-6 pt-5">
        <form onSubmit={handleCitySubmit} className="relative mb-2">
          ...
          <input
            value={cityInput}
            onChange={(e) => { setCityInput(e.target.value); setCityNotFound(false); }}
            placeholder={`Currently browsing ${city.label} — search any US city...`}
        ...
        {cityNotFound && (
          <p className="text-[11px] text-[#B5AFA0] mb-2 leading-relaxed">
            We don't have real shop data loaded for "{cityInput}" yet in this prototype — try one of the cities
            below. In production, Perch would query the Places API live for any US city instead of a fixed list.
          </p>
        )}
